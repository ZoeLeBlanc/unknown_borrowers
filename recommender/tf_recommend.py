...
top100_train_accuracy = train_accuracy["factorized_top_k/top_100_categorical_accuracy"]
top100_test_accuracy = test_accuracy["factorized_top_k/top_100_categorical_accuracy"]
print(f"Top-100 accuracy (train): {top100_train_accuracy:.2f}.")
print(f"Top-100 accuracy (test): {top100_test_accuracy:.2f}.")

top10_train_accuracy = train_accuracy["factorized_top_k/top_10_categorical_accuracy"]
top10_test_accuracy = test_accuracy["factorized_top_k/top_10_categorical_accuracy"]
print(f"Top-10 accuracy (train): {top10_train_accuracy:.2f}.")
print(f"Top-10 accuracy (test): {top10_test_accuracy:.2f}.")
...
affinity_scores, titles = index(np.array(["hemingway"]))
print(
    f"Top 5 recommendations for hemingway: {', '.join(t.numpy().decode() for t in titles[0][:5])} ({', '.join(str(s.numpy()) for s in affinity_scores[0][:5])})"
)
